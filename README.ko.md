<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 102 프로젝트 배너" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 102｜치유 기하학 저널

### 사진을 맑고 차분하며 생명력 있는 기하학으로 추출합니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 102이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

**Panel 102**는 사진 속 주체의 식별성은 유지하면서 복잡한 배경, 사실적 재현, 값싼 필터를 벗어나고 싶을 때 적합합니다. 핵심 윤곽, 구조, 자세, 관계를 소수의 기하 색면으로 추출해 맑은 여백 속에 고요하고 현대적이며 생명력 있는 치유계 편집 포스터를 만듭니다.

### 이런 경우에 적합합니다

- 정보가 많은 사진에서 가장 식별하기 쉬운 윤곽, 구조, 자세, 관계만 남겨 명확하게 만들고 싶을 때
- 양형과 음형, 위계, 넓은 여백으로 차갑지 않은 현대적 고급감을 만들고 싶을 때
- 탁하고 값싼 필터색을 원본에서 가져온 밝고 깨끗하며 따뜻한 색으로 바꾸고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 102로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 기하학적 추상 · 평면 색 · 명확한 윤곽 · 양형과 음형 · 넓은 여백 · 치유계 편집 시스템
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 소수의 기하 형태, 맑은 색면, 이성적인 여백으로 사진의 기억을 담는 현대 편집 포스터 |
| **대표 특징** | 기하학화된 주체, 평면 색면, 명확한 윤곽, 가는 미세 주석, 옅은 배경, 넓은 여백 |
| **입력 존중 방식** | 원래 대상의 핵심 특징, 자세, 관계를 보존해 디자인 부분도 현재 사진과 즉시 연결됨 |
| **활용처** | 브랜드 콘텐츠, 전시·건축 포스터, 인물·반려동물 기념, 표지, 소셜 콘텐츠, 기기 벽지 |
<!-- xxd-panel-benefit:end -->

## 변환 논리

```text
주체의 핵심 윤곽·구조·자세 식별 → 부차적 디테일 삭제 → 소수의 기하 형태와 평면 색면으로 압축 → 양형·음형, 시각 무게중심, 여백 리듬 조정 → 원본에서 밝고 맑은 치유색 추출 → 절제된 제목·번호·미세 주석으로 편집 질서 완성
```

## 완성작의 특징

- 소수의 기하 형태, 명확한 윤곽, 가는 선 관계로 정제해도 주체의 핵심 정체성과 자세는 식별됩니다.
- 시각 무게중심, 주종 위계, 양형과 음형, 이미지·글자 균형으로 비어 있지 않고 고요한 구도를 만듭니다.
- 미색 또는 지능적으로 맞춘 옅은 배경에 넓은 여백을 남겨 주체를 정리된 기억 표본처럼 놓습니다.
- 원본에서 가져온 밝고 깨끗하며 따뜻한 색면은 선명하지만 자극적이지 않고 탁함, 어두움, 형광색, 값싼 캔디색을 피합니다.
- 제목, 번호, 미세 주석은 적고 정교하며, 사실주의, 만화, 플라스틱 질감, 템플릿 장식을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-102-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 102의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-102 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-102 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-102 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-102 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-102 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-102 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 102 적용, 공통 설정은 한 번만 확정
/xxd-panel-102 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-102`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-102-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-102-prompt.zh-CN.md)
...
| **[xxd-panel-102](https://github.com/nevertoday/xxd-panel-102)** | 치유 기하학 저널 · 기하학화된 주체 · 맑은 평면 색 · 양형과 음형 · 넓은 여백 · 절제된 미세 주석 |
...
### Xiaoxiaodong 총괄 · 장군 총지휘 Skill · CNY 102

CNY 102을 한 번 결제하면 이 시리즈의 장군 총지휘 Skill(`xxd-panel-all`)을 사용할 수 있습니다. 전체 병사 Skills의 통합 관리, 추천, 지정 파견, 일괄 조정을 지원합니다. WeChat 메시지에 “장군 총지휘 Skill”이라고 적어 주세요.
...
| **병사급** | `xxd-panel-NNN` (현재 001–102) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
