<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 102 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 102 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
识别主体的核心轮廓、结构与姿态 → 删除次要细节 → 压缩成少量几何形与平面色块 → 调整正负形、视觉重心与留白节奏 → 从原图提炼明亮清透的治愈色 → 以克制标题、编号或微注释完成编辑秩序
```

## 成品中最容易识别的风格特点

- 主体被提炼成少量几何形、清楚轮廓与细线关系，但核心身份和姿态仍然可辨。
- 构图重视视觉重心、主次层级、正负形与图文平衡，画面安静而不是空洞。
- 米白或智能匹配的极浅色底保留大面积留白，让主体像一枚被整理好的记忆标本。
- 明亮、纯净、温暖的照片来源色块鲜明但不刺眼；避开灰脏、暗沉、荧光与廉价糖果色。
- 标题、编号和微注释少而精致，具有设计工作室与展览海报气质；拒绝写实插画、卡通、塑料质感和模板装饰。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-102-prompt.en.md)
...
102 个 Panel 各自保留独立的原始提示词与审美逻辑。下面列出全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **[xxd-panel-102](https://github.com/nevertoday/xxd-panel-102)** | 治愈几何志 · 几何化主体 · 平面清透色块 · 正负形关系 · 大面积留白 · 克制微注释 |
...
### 小小东总控 · 将军总指挥 Skill · 102 元

一次支付 102 元获得本套将军总指挥 Skill（`xxd-panel-all`），用于主控、推荐、点将和批量调度全部士兵 Skills。扫码后请备注“将军总指挥 Skill”。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–102） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
