<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 102 プロジェクトバナー" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 102｜癒やしの幾何学誌

### 写真を静かで明快な、生命感のある幾何学へ
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 102 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

**Panel 102** は写真の主体の識別性を保ちながら、雑多な背景、写実的な描き直し、安価なフィルターから離れたい場合に適しています。中心的な輪郭、構造、姿勢、関係を少数の幾何学的な色面へ抽出し、澄んだ余白の中に静かで現代的、生命感のある癒やし系編集ポスターを作ります。

### 向いている用途

- 情報量の多い写真から、最も識別しやすい輪郭、構造、姿勢、関係だけを残して明快にしたい場合
- 正負形、階層、大きな余白で、冷たくない現代的な上質感を作りたい場合
- 濁った安価なフィルター色を、元写真由来の明るく純粋で温かな色に置き換えたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 102 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 幾何学的抽象 · 平面色 · 明瞭な輪郭 · 正負形 · 大きな余白 · 癒やし系編集システム
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 少数の幾何形、澄んだ色面、理性的な余白で写真の記憶を支える現代編集ポスター |
| **識別しやすい特徴** | 幾何学化された主体、平面色、明瞭な輪郭、細い微注釈、淡い背景、大きな余白 |
| **入力の尊重** | 元の物体の中心的な特徴、姿勢、関係を保ち、デザイン部分も現在の写真とひと目で対応する |
| **用途** | ブランドコンテンツ、展覧会・建築ポスター、人物・ペットの記念、表紙、SNS、端末壁紙 |
<!-- xxd-panel-benefit:end -->

## 変換ロジック

```text
主体の中心的な輪郭、構造、姿勢を特定 → 二次的な細部を削除 → 少数の幾何形と平面色へ圧縮 → 正負形、視覚的重心、余白のリズムを調整 → 元写真から明るく澄んだ癒やし色を抽出 → 控えめなタイトル、番号、微注釈で編集秩序を完成
```

## 完成作の特徴

- 少数の幾何形、明瞭な輪郭、細線の関係に抽出しても、主体の中心的な同一性と姿勢は識別できます。
- 視覚的重心、主従階層、正負形、図文バランスを整え、空虚ではなく静かな構図にします。
- 生成り色または知的に合わせた淡い背景に大きな余白を取り、主体を整理された記憶標本のように置きます。
- 元写真由来の明るく純粋で温かな色面は明快でも刺激的ではなく、濁り、暗さ、蛍光色、安価なキャンディー感を避けます。
- タイトル、番号、微注釈は少なく精緻にし、写実、漫画調、プラスチック感、テンプレート装飾を避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-102-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 102 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-102 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-102 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-102 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-102 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 102 を適用し、共通設定は一度だけ解決
/xxd-panel-102 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-102`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-102-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-102-prompt.zh-CN.md)
...
| **[xxd-panel-102](https://github.com/nevertoday/xxd-panel-102)** | 癒やしの幾何学誌 · 幾何学化主体 · 澄んだ平面色 · 正負形 · 大きな余白 · 控えめな微注釈 |
...
### Xiaoxiaodong 総控 · 将軍総指揮 Skill · CNY 102

CNY 102 の一回払いで、このシリーズの将軍総指揮 Skill（`xxd-panel-all`）を利用できます。全兵士 Skills の統括、推薦、指名派遣、一括調整に対応します。WeChat では「将軍総指揮 Skill」と記載してください。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–102） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
